...
function setupEventListeners() {
  ...
  guestDropzone.addEventListener('drop', (e) => { e.preventDefault(); openAuthModal(false); });
...
function setupFileHandling() {
  function handleFiles(list) {
    ...
    for (const file of incoming) {
        ...
        // 2. Check strict allow list (Blocks HEIC/PDF/TIFF)
        if (!ALLOWED_MIME_TYPES.includes(file.type)) {
            alert(`Skipped "${file.name}": File type ${file.type} not supported on web. Use CLI for HEIC.`);
            continue;
        }
...
function renderPreviews() {
  ...
  filesToUpload.forEach((file, idx) => {
    ...
    btn.innerHTML = '&times;';
    btn.onclick = (e) => { e.stopPropagation(); filesToUpload.splice(idx, 1); renderPreviews(); };
    div.appendChild(img);
    div.appendChild(btn);
...
async function handleUpload() {
  ...
  for (const file of filesToUpload) {
    try {
      ...
      const finalRes = await apiFetch('/api/v1/upload/complete', {
        method: 'POST',
        body: { iid: reqRes.iid, key: reqRes.key, filename: file.name, mime_type: file.type }
      });
  ...
  setTimeout(() => { progressBar.classList.remove('active'); progressBarFill.style.width = '0'; refreshGallery(); }, 1000);
}
...
function addResultLink(url, file) {
  ...
  btn.onclick = () => { input.select(); document.execCommand('copy'); btn.textContent = 'Copied!'; setTimeout(() => btn.textContent = 'Copy', 1500); };
  div.appendChild(img);
...
async function apiFetch(endpoint, opts = {}) {
  ...
  opts.headers = { ...opts.headers, 'X-API-Key': key, 'Content-Type': 'application/json' };
